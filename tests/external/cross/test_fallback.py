"""External tests for provider fallback logic.
    ...
    async def test_cerebras_fails_openai_succeeds(self, openai_api_key: str) -> None:
        ...
        with patch.dict(os.environ, env, clear=True):
            config = AgentConfig(
                ...
                fallback=FallbackConfig(model=Model.GPT_6_LUNA),  # OpenAI fallback
                enable_todo=False,
            )
    ...
    async def test_openai_fails_cerebras_succeeds(self, cerebras_api_key: str) -> None:
        ...
        with patch.dict(os.environ, env, clear=True):
            config = AgentConfig(
                system_prompt="You are a helpful assistant. Reply concisely.",
                tools=[],
                model=Model.GPT_6_SOL,  # OpenAI model
    ...
    async def test_both_models_fail(self) -> None:
        ...
        with patch.dict(os.environ, env, clear=True):
            config = AgentConfig(
                ...
                fallback=FallbackConfig(model=Model.GPT_6_LUNA),  # Will also fail
                enable_todo=False,
            )
            agent = Agent(config=config)
            ...
            assert "gpt-6-luna" in error.fallback_model
    ...
    async def test_streaming_fallback(self, openai_api_key: str) -> None:
        ...
        with patch.dict(os.environ, env, clear=True):
            config = AgentConfig(
                ...
                tools=[],
                model=Model.CEREBRAS_GPT_OSS_120B,
                fallback=FallbackConfig(model=Model.GPT_6_LUNA),
                enable_todo=False,
            )
            agent = Agent(config=config)
...
class TestStickyFallbackWithSession:
    ...
    @pytest.mark.asyncio
    async def test_sticky_fallback_stays_on_fallback(self, openai_api_key: str) -> None:
