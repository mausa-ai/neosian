"""`neosian import` is a verbatim pass-through to the one argv grammar."""
...
from neosian._cli.main import import_
...
def test_arguments_forward_verbatim_behind_the_verb(
    ...
    with pytest.raises(typer.Exit) as excinfo:
        import_(ctx)  # type: ignore[arg-type]
    assert excinfo.value.exit_code == 0
    assert seen == [["import", "archive", "--root", "m", "--scope", "user:me"]]
