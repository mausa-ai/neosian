...
| OpenAI | `OPENAI_API_KEY` | `gpt-6-sol` (default), `gpt-6-astra`, `gpt-6-luna`, `gpt-5.1` (reasoning, `max` effort), on the Responses API with reasoning carried through tool loops and nothing stored at the provider |
| Anthropic | `ANTHROPIC_API_KEY` | `claude-fable-5-1`, `claude-opus-5-5`, `claude-sonnet-5` (default); vision/PDF input, prompt caching, adaptive thinking |
...
(`AgentConfig(model="gpt-6-sol")`); each carries its provider's
