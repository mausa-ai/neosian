...
| `OPENAI_API_KEY` | OpenAI (`gpt-6-astra`, the GPT-6 family and `gpt-5.1`, reasoning; the Responses API) | GPT models leave the router; `external_openai` self-skips; `chat` and `playground` refuse a GPT model before its SDK (`MissingAPIKeyError` naming the var) |
