...
- `examples/eval_memory_baseline.yaml`: sha256
  ...
  *(NK post-close, 2026-09-04, the `skills` scenario's three turns
  ...
  ruling the same day: no re-dispatch for this, so the rows stand as
  ...
  *(NB post-close, 2026-09-03, `cross-client` re-shaped after dispatch
  ...
  columns, paraphrasing past the word, so the detail is now unreachable
...
- **† Cerebras's hourly cap ended the lane:** the last cell (mcp ×
  ...
  hour limit exceeded" at 17:24 UTC, the harness level, not a cell, so
...
Findings, recorded as found, on the first board whose reds are read from
the bytes:
...
- **The three cells NZ works last, settled.** OpenAI's
  ...
  OpenAI's `long-horizon-recall`: green on all four columns, so the
...
- **`skills` without frontmatter is the largest class on the board:**
  ...
  function (it asked a clarifying question before saving, so the turn's
  response pin, the store untouched). The NK ruling stands.
- **Sonnet 39/40 + the axis 19/20:** on function `correct-wrong-memory`,
...
- **The calibration held.** `reflection-close` is green everywhere it
  ran, gpt-5.1 on both transports included, with the refused token
...
- **The branch-fact cell misfired on two providers.**
  ...
  the stored bytes cannot be inspected post-hoc, since CI runners discard
...
- **OpenAI `reflection-close` (function): a second `/user` document,
  ...
  write-discipline ruling extends to the reflection boundary, so
...
unruled, because tuning a scenario to make a provider pass is never silent):
...
- **ReflectionBatch strict-mode 400 (run 1, fixed).** OpenAI's strict
  ...
  `reflection-close` cells still passed, the model legitimately writing
...
- **Provenance annotations**: "Lives in Lisbon (moved from Paris)" is a
  good update, so the `forbidden:` pins on superseded values dropped.
- **Filing granularity**: one consolidated project doc vs two separate
  docs are both disciplined, so distractor pins became content presence,
  not document counts.
