...
environment); `--scope` here is a raw scope, never a mount, and
`--actor` is a *filter* (the audit writes nothing). Exit tiers as
...
from neosian._foundation.shared.exceptions import MemoryScopeInvalidError, NeosianError
...
async def run(
    ...
    *,
    ...
    parser.add_argument("--scope", required=True, help="the scope to audit")
